...
export default function VelocityPaintIridescence() {
  ...
  const controls = useControls('VelocityPaintIridescence', {
    paint: folder({
    ...
    iridescence: folder({
      ...
      filmThickness:     { value: 2.5,   min: 0.1, max: 10,  step: 0.05, label: 'Film Thickness' },
      iridIntensity:     { value: 4,     min: 0,   max: 15,  step: 0.1,  label: 'Irid Intensity' },
      fresnelPower:      { value: 2.0,   min: 0.5, max: 8,   step: 0.1,  label: 'Fresnel Power' },
      flowFreq:          { value: 6,     min: 1,   max: 30,  step: 0.5,  label: 'Flow Freq' },
      weightFreq:        { value: 1.5,   min: 0.01, max: 6,  step: 0.01, label: 'Weight Freq' },
      velocityScale:     { value: 8,     min: 0,   max: 20,  step: 0.05, label: 'Velocity Scale' },
      edgeShade:         { value: 3,     min: 0,   max: 5,   step: 0.05, label: 'Edge Shade' },
      viscosity:         { value: 0.6,   min: 0,   max: 1,   step: 0.01, label: 'Viscosity' },
      saturation:        { value: 1.3,   min: 0.1, max: 3,   step: 0.05, label: 'Saturation' },
      bgOpacity:         { value: 1,     min: 0,   max: 1,   step: 0.01, label: 'BG Opacity' },
    }),
    backgroundColor: { value: '#060810', label: 'Background' },
  });
  ...
  useEffect(() => {
    ...
    engine.config.saturation          = controls.saturation;
  ...
  useEffect(() => {
    ...
    const engine = new VelocityPaintIridescenceEngine(gl);
    engine.setDPR(dpr);
    ...
    const onCtxRestored = () => {
      ...
      const newEngine = new VelocityPaintIridescenceEngine(newGl);
