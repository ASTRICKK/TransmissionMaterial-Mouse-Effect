/*
  VelocityPaintIridescence Engine
  Iridescence variant of VelocityPaint.
  Same velocity-painting pipeline, different compositing shader
  that simulates thick-film interference with higher refractive
  index (n≈1.5), triple-layer reflections, and viscous flow
  for the deep purple-teal-blue colors of oil on water.
*/

import { VERT } from './shaders/vert';
import { PAINT_FRAG, PAINT_FRAG_NOISE, PAINT_FRAG_NOISE_2 } from './shaders/paintFrag';
import { IRIDESCENCE_FRAG } from './shaders/iridescenceFrag';
...
export interface VPIridescenceConfig {
  // Brush / Velocity
  pushStrength: number;
...
export const VP_IRIDESCENCE_DEFAULTS: VPIridescenceConfig = {
  pushStrength: 25,
  ...
  maxRadius: 100,
  radiusRange: 80,
  // Iridescence defaults — tuned for oil-slick iridescence
  distortionAmount: 8,
  filmThickness: 2.5,
  iridIntensity: 4,
  fresnelPower: 2.0,
...
export class VelocityPaintIridescenceEngine {
  ...
  constructor(gl: WebGL2RenderingContext, config?: Partial<VPIridescenceConfig>) {
    this.gl = gl;
    this.config = { ...VP_IRIDESCENCE_DEFAULTS, ...config };
  ...
  private buildShaders() {
    ...
    this.iridescenceShader = createShaderProgram(gl, VERT, IRIDESCENCE_FRAG);
  ...
  private renderPass() {
    ...
    if (this.config.distortionAmount > 0) {
      gl.useProgram(this.iridescenceShader.program);
      gl.uniform1i(this.iridescenceShader.uniforms.t_scene,    this.rtScene.attach(0));
      gl.uniform1i(this.iridescenceShader.uniforms.t_velPaint, this.rtCurr.attach(1));
      gl.uniform2f(this.iridescenceShader.uniforms.v_paintTexel,
        1.0 / this.rtCurr.width, 1.0 / this.rtCurr.height);
      gl.uniform1f(this.iridescenceShader.uniforms.p_stepScale,
        this.config.distortionAmount * this.config.velocityScale * 0.25);
      gl.uniform1f(this.iridescenceShader.uniforms.p_filmThickness, this.config.filmThickness);
      gl.uniform1f(this.iridescenceShader.uniforms.p_iridIntensity, this.config.iridIntensity);
      gl.uniform1f(this.iridescenceShader.uniforms.p_fresnelPower,  this.config.fresnelPower);
      gl.uniform1f(this.iridescenceShader.uniforms.p_edgeShade,     this.config.edgeShade);
      gl.uniform1f(this.iridescenceShader.uniforms.p_flowFreq,      this.config.flowFreq);
      gl.uniform1f(this.iridescenceShader.uniforms.p_weightFreq,    this.config.weightFreq);
      gl.uniform1f(this.iridescenceShader.uniforms.p_bgOpacity,     this.config.bgOpacity);
      gl.uniform1f(this.iridescenceShader.uniforms.p_time,          this.now * 0.001);
      gl.uniform1f(this.iridescenceShader.uniforms.p_viscosity,     this.config.viscosity);
      gl.uniform1f(this.iridescenceShader.uniforms.p_saturation,    this.config.saturation);
      this.drawQuad(null);
    } else {
  ...
  dispose() {
    ...
    gl.deleteProgram(this.iridescenceShader.program);
